<!DOCTYPE html>
<html>
 <head>
  <meta charset="utf-8" />
  <meta http-equiv="X-UA-Compatible" content="IE=edge" />
  <meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no" />
  <meta name="format-detection" content="telephone=no" />
  <title>设计公司,广州设计公司,广州包装设计公司,广州标志设计公司,广州市赤风设计有限公司简介</title>
  <meta name="robots" content="all" />
  <meta name="author" content="redwind design" />
  <meta name="Copyright" content="Copyright 2001-2015 All Rights Reserved Redwind design co.,ltd." />
  <meta name="description" content="国内顶级品牌设计公司,赤风设计公司成立于2001年，是专业的vi设计公司，标志设计公司，包装设计公司，商标设计公司和品牌设计公司,为客户提供从品牌标志设计到品牌全案设计的专业服务" />
  <meta name="keywords" content="设计公司,广州设计公司,标志设计公司,包装设计公司,VI设计公司,,商标设计公司,logo设计公司,广州标志设计公司，广州VI设计公司，广州包装设计公司，广州商标设计公司，广州logo设计公司，赤风设计" />
  <meta name="searchtitle" content="设计公司,广州设计公司,专业设计公司,标志设计公司,包装设计公司,VI设计公司,商标设计公司,logo设计公司,广州标志设计公司，广州VI设计公司，广州包装设计公司，广州商标设计公司，广州logo设计公司，赤风设计" />
  <link href="/Public/images/favicon.ico" rel="shortcut icon" />
  <link rel="stylesheet" type="text/css" href="css/font.css" />
  <link rel="stylesheet" type="text/css" href="css/base.css" />
  <link rel="stylesheet" type="text/css" href="css/rewind.css" />
  <script src="js/jquery-1.11.0.min.js"></script>
  <!--[if lt IE 9]><script src="js/html5shiv.min.js"></script><script src="js/respond.min.js"></script><![endif]-->
  <script charset="utf-8" src="other/wpa.php"></script>
 </head>
 <body>
  <!-- header -->
  <div class="header">
   <a href="/index.html"><img class="logo1" src="images/logo.jpg" title="赤风品牌策划与设计" /></a>
   <img class="navBtn" src="images/navBtn.png" title="点我!点我!" />
   <div class="container">
    <ul class="nav">
     <a href="./index.html"><li>首页</li></a>
     <a href="./superiority.html"><li>国家品牌智库</li></a>
     <a href="./project.html"><li>评审标准</li></a>
     <a href="./project.html"><li>申报流程</li></a>
     <a href="javascript:void(0)"><li class="on navBtn_">申报表索取</li></a>
     <a href="./news.html"><li>品牌明星</li></a>
     <a href="./partner.html"><li>品牌总裁</li></a>
     <a href="./contact.html"><li>联系我们</li></a>
    </ul>
   </div>
  </div>
  <!-- header over-->
  <div class="stance"></div>
  <!-- list -->
  <div class="list">
   <div class="container">
    <div class="left">
     <span class="redTitle">Hi,are you ready?</span>
     <p class="title">申报期限将近<br />请抓紧登录“申报通道”</p>
     <p class="info">有一个品牌项目想和我们谈谈吗?您可以填写右边的表格，让我们了解您的项目需求，这是一个良好的开始，我们将会尽快与你取得联系。当然也欢迎您给我们写信或是打电话，让我们听到你的声音!</p>
     <div class="footTitle">
      <span class="titleB"></span>
      <span>申报服务专员</span>
     </div>
     <p class="info">电 话：+021-63453030</p>
    </div>
    <div class="right">
     <p class="title">申报通道</p>
     <p class="line"><input type="text" class="inputText" id="listCompany" value="公司名称" /></p>
     <p class="line"><input type="text" class="inputText" id="listShop" value="品牌名称" /></p>
     <p class="line"><input type="text" class="inputText" id="listName" value="联系人" /></p>
     <p class="line"><input type="text" class="inputText" id="listPhone" value="联系电话" /></p>
     <p class="line"><input type="text" class="inputText" id="listEmail" value="联系邮箱" /></p>
     <p class="line"><input type="button" onclick="contactSubmit('/Home/Public/contact_submit.html',$('#listName').val(),$('#listCompany').val(),$('#listEmail').val(),$('#listTitle').val(),$('#listInfo').val())" class="inputBtn" value="立即提交" /></p>
    </div>
   </div>
   <img id="listCloseBtn" src="images/closeIcon.png" />
  </div>
  <!-- list over -->
  <!-- banner -->
  <div class="banner" style="background-image:url(Public/upload/banner/other/1433569313.jpg);">
   <div class="bannerInfo">
    <p class="title">REDWIND</p>
    <p class="title"> CHINA’S TOP 50 BRAND DESING COMPANIES</p>
    <span class="titleCh">中国品牌设计公司50强</span>
   </div>
   <img class="bannerBg" src="images/bannerBg.jpg" />
  </div>
  <!-- banner over -->
  <!-- main -->
  <div class="main">
   <!-- mainChild1 -->
   <div class="mainChild" id="mainChild1">
    <div class="container">
     <div class="sub"></div>
     <p class="mainChildTitle">成熟可靠的品牌建设伙伴</p>
     <div class="content">
      <p class="info">广州市赤风设计有限公司自2001年开始品牌设计之路， 2017年也迎来了公司创立16周年。<br /> 我们以品牌创建、品牌更新以及品牌管理作为核心业务模块。进行以创意策略，视觉概念和以结果为导向的品牌沟通。我们的团队拥有赋有才华的设计师和各领域跨界品牌合作伙伴，品牌或公司的价值是由独特的交互沟通手段构成 。强大的品牌在所有的表现形态中具有一致的外观。所以我们提出量身定制的品牌解决方案和创新的现代设计。并与客户分享各行业品牌的服务经验！ </p>
      <p class="info">无论是对内还是对外，品牌感知度更高，识别度也更大。我们坚信整合的品牌方法论。并通过商业模式来定义品牌模式，让客户通过品牌建立与传播提高业绩。众多品牌与我们一起实现新的品牌战略，视觉概念和实质性的创新成果，查看我们的案例，了解一下我们不同领域的客户以及我们为广大客户实现的结果 ！</p>
      <div class="contentMun">
       <div class="child">
        <p class="mun"><span>2000</span>+</p>
        <p>超过2000次服务经验</p>
       </div>
       <div class="child">
        <p class="mun"><span>16</span>+</p>
        <p>16年专业历程</p>
       </div>
       <div class="child">
        <p class="mun">TOP<span>50</span></p>
        <p>中国品牌设计50强</p>
       </div>
      </div>
     </div>
     <div class="sub"></div>
     <p class="mainChildTitle">我们的理念</p>
     <p class="info">赤风设计坚信整体的设计方法论。品牌或公司的价值是由独特的交互沟通手段构成 。<br />凭借对市场趋势敏锐的洞察和对消费者、企业形态深刻的理解，让赤风获得丰富的品牌设计灵感，这正是我们给予每位客户的期待，赤风致力成为一家优秀的品牌策划与设计机构，我们帮助品牌成长，为品牌提升价值是赤风的企业理念。</p>
    </div>
   </div>
   <!-- mainChild1 over -->
   <!-- mainChild2 -->
   <div class="mainChild" id="mainChild2" style="background-color: #dfddde;">
    <a href="/project.html">
     <div class="container" style="background-image:url(Public/images/rewind/idea.gif);"></div></a>
   </div>
   <!-- mainChild2 over -->
   <!-- mainChild3 -->
   <div class="mainChild mainChildList" id="mainChild3">
    <div class="container">
     <div class="wapImg">
      <img src="images/wapmain1.jpg" />
     </div>
     <div class="left">
      <img src="images/main1.jpg" />
     </div>
     <div class="right">
      <div class="head">
       <p class="title">十六年激情永不止步</p>
       <p class="titleEn">Sixteen years keep walking</p>
      </div>
      <div class="content">
       <p>十六年的风雨,由个人工作室一走到现在，当中离不开赤风同仁顽强的创业精神，离不开伴随我们同行的客户与专业的合作伙伴，赤风远行这是一份期待、一个目标与希望.....它将激励着我们迈过更多个十六年!</p>
      </div>
      <a href="/history.html">
       <div class="linkBtn">
        <span>了解更多信息</span>
       </div></a>
     </div>
    </div>
   </div>
   <!-- mainChild3 over -->
   <!-- mainChild4 -->
   <div class="mainChild mainChildList" id="mainChild4">
    <div class="container">
     <div class="wapImg">
      <img src="images/wapmain2.jpg" />
     </div>
     <div class="right">
      <img src="images/main2.jpg" />
     </div>
     <div class="left">
      <div class="head">
       <p class="title">赤风精英品牌团队</p>
       <p class="titleEn">Redwind elite brand team</p>
      </div>
      <div class="content">
       <p>我们的团队成员不追求数量、追求质量 。我们经验丰富并且专业，我们之间合作时就好像一个人，协同一致毫无保留。我们珍视想法，同时也看重过程转化带来的冲击力和影响力，在我们眼中，任何细节都不容小觑。</p>
      </div>
      <a href="/team.html">
       <div class="linkBtn">
        <span>了解更多信息</span>
       </div></a>
     </div>
    </div>
   </div>
   <!-- mainChild4 over -->
   <!-- mainChild5 -->
   <div class="mainChild" id="mainChild5">
    <div class="container">
     <p class="mainChildTitle">不大不小刚刚好</p>
     <div class="sub"></div>
     <p class="info">创作是一项脑力劳动，赤风的自由开放除了体现在我们的办公环境上。 工作氛围亦是如此， 我们没有豪华的办公装修&quot;红色与黑色&quot;依然是我们装饰的主色调，一切都是那么自自然然，简简单单....</p>
     <div class="contents">
      <div class="content">
       <img src="images/1433567911.jpg" title="设计公司环境" />
      </div>
      <div class="content">
       <img src="images/1433322493.jpg" title="设计公司环境" />
      </div>
      <div class="content">
       <img src="images/1433567111.jpg" title="设计公司环境" />
      </div>
      <div class="content">
       <img src="images/1433927276.jpg" title="设计公司环境" />
      </div>
      <div class="content">
       <img src="images/1433323038.jpg" title="设计公司环境" />
      </div>
      <div class="content">
       <img src="images/1433322603.jpg" title="设计公司环境" />
      </div>
      <div class="content">
       <img src="images/1433322931.jpg" title="设计公司环境" />
      </div>
      <div class="content">
       <img src="images/1433322683.jpg" title="设计公司环境" />
      </div>
      <div class="content">
       <img src="images/1433322757.jpg" title="设计公司环境" />
      </div>
      <div class="content">
       <img src="images/1433322831.jpg" title="设计公司环境" />
      </div>
      <img class="mainChild5Btn leftBtn" onclick="mainChild5BtnClick('-')" src="Public/images/rewind/leftBtn.png" />
      <img class="mainChild5Btn rightBtn" onclick="mainChild5BtnClick('+')" src="Public/images/rewind/rightBtn.png" />
     </div>
     <div class="navbtns"></div>
    </div>
   </div>
   <!-- mainChild5 over -->
   <!-- mainChild6 -->
   <div class="mainChild" id="mainChild6" style="background-image:url(Public/images/rewind/banner2.jpg);">
    <p class="mainChildTitle">要么做到 要么别承诺</p>
    <div class="sub"></div>
    <p class="mainChildTitle2">赤风文化</p>
    <p class="titleEn">CAN’T DO</p>
    <p class="titleEn">DON’T MAKE PROMISE</p>
   </div>
   <!-- mainChild6 over -->
   <!-- mainChild7 -->
   <div class="mainChild" id="mainChild7">
    <div class="container">
     <p class="mainChildTitle">合适的才是最好的</p>
     <div class="sub"></div>
     <p class="mainChildTitle2">品牌观点</p>
     <p class="info">一个独一无二的赤风，让我们真诚面对自公司成立以来就一直激励我们的传统和价值观，作为专业的品牌设计公司，赤风不遗余力地为客户提供令同业望其项背的服务。我们立志做企业最可信赖的品牌设计管理商，一个真正能让设计为企业带来利润的品牌设计机构，为您解决品牌运营所遇到的问题是赤风存在的价值和企业理念。</p>
     <div class="content">
      <div class="quan" id="quan1">
       <p class="titleEn">Ustability</p>
       <p class="titleCh">实用</p>
      </div>
      <div class="quan" id="quan2">
       <p class="titleEn">Crestivity</p>
       <p class="titleCh">创意设计</p>
      </div>
      <div class="quan" id="quan3">
       <p class="titleEn">Market</p>
       <p class="titleCh">营销</p>
      </div>
      <div class="quan" id="quan4">
       <p class="titleEn">Balance</p>
       <p class="titleCh">平衡</p>
      </div>
     </div>
    </div>
   </div>
   <!-- mainChild7 over -->
  </div>
  <!-- main over -->
  <!-- contact -->
  <div class="contact">
   <div class="container">
    <p class="mainTitle">Are you interested in ?</p>
    <p class="title">感兴趣吗？</p>
    <div class="mainSub"></div>
    <p class="info">有关我们服务的更多信息,请联系</p>
    <p class="info">138 2619 7540 吴先生 / 138 2629 9018 曾先生</p>
   </div>
  </div>
  <!-- contact over -->
  <!-- footerNav -->
  <div class="footerNav">
   <div class="container">
    <div class="right">
     <p class="title">与我们<span>合作</span></p>
     <p class="info">与赤风合作，您将会得到更成熟的品牌建设服务。我们以客户至上，同时也相互挑战，力求呈现最好的品牌建设成果。</p>
     <p class="callTitle">品牌顾问热线(曾生):</p>
     <p class="callMun">+138 2629 9018</p>
    </div>
    <div class="left">
     <ul>
      <li class="title">优 势</li>
      <a href="/superiority.html"><li>赤风优势</li></a>
      <a href="/model.html"><li>品牌模式</li></a>
      <a href="/superiority.html"><li>服务项目</li></a>
      <a href="/superiority.html"><li>服务流程</li></a>
      <a href="/customer.html"><li>客户列表</li></a>
     </ul>
     <ul>
      <li class="title">案 例</li>
      <a href="project.html"><li>全部案例</li></a>
      <a href="/Home/Project/index.html"><li>服务分类</li></a>
      <a href="/Home/Project/index.html"><li>行业分类</li></a>
     </ul>
     <ul>
      <li class="title">赤 风</li>
      <a href="/redwind.html"><li>赤风介绍</li></a>
      <a href="/history.html"><li>发展历程</li></a>
      <a href="/team.html"><li>核心团队</li></a>
      <a href="/redwind.html"><li>办公环境</li></a>
      <a href="/redwind.html"><li>公司文化</li></a>
      <a href="/redwind.html"><li>品牌观点</li></a>
     </ul>
     <ul>
      <li class="title">风 向</li>
      <a href="/news/1.html"><li>赤风动态</li></a>
      <a href="news/2.html"><li>行业资讯</li></a>
     </ul>
     <ul>
      <li class="title">伙 伴</li>
      <a href="/job.html"><li>招聘信息</li></a>
      <a href="/partner.html"><li>合作伙伴</li></a>
     </ul>
     <ul>
      <li class="title">联 系</li>
      <a href="/contact.html"><li>联系方式</li></a>
      <a href="/contact.html"><li>位置地图</li></a>
      <a href="/contact.html"><li>留言板</li></a>
     </ul>
    </div>
   </div>
  </div>
  <!-- footerNav over -->
  @extends('common.footer')
  <script src="{{ URL::asset('assets/js/newWeb/redwind.js')}}"></script>
</html>